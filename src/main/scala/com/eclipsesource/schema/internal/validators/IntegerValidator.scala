...
object IntegerValidator extends SchemaTypeValidator[SchemaInteger] with NumberConstraintsValidator {
  ...
  val isInt: scalaz.Reader[(NumberConstraints, ResolutionContext), Rule[JsValue, JsValue]] =
  ...
  override def validate(schema: SchemaInteger, json: => JsValue, context: ResolutionContext): VA[JsValue] = {
    val reader = for {
      maxRule <- validateMax
      minRule <- validateMin
